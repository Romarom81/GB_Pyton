...
def init_game (update, context):
    ...
    quallity_candies = int(update.message.text)
...
def next_move (update, context):
    global quallity_candies
    qulity_candies_pleyer = int(update.message.text)
    ...
    if quallity_candies >= 29:
        ...
        if quallity_candies >= 29:
            ...
            update.message.reply_text (' Поздравляю, Твоя победа!!', reply_markup=markup)
            
    else:
        update.message.reply_text (' Моя победа', reply_markup=markup)
